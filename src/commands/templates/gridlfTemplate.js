...
function generateScrobbleImage(
    item,
    x,
    y,
    width,
        ...
        if (media_type === "tracks") {
            if (param === "noplays") {
                textElements.push({
                    ...
                    font:
                        `normal 700 ${config.TRACK_NAME_FONT_SIZE}px "Noto Sans JP", sans-serif`,
                });
                textElements.push({
                    text: limitText(item.track.artist, 30),
                    yOffset: config.SCROBBLE_TEXT_Y_OFFSET,
                    font:
                        `normal 500 ${config.TRACK_ARTIST_FONT_SIZE}px "Noto Sans JP", sans-serif`,
                });
            } else {
                textElements.push({
                    text: limitText(item.track.name, 50),
                    yOffset: config.SCROBBLE_TEXT_Y_OFFSET +
                        2 * config.TEXT_LINE_HEIGHT,
                    font:
                        `normal 700 ${config.TRACK_NAME_FONT_SIZE}px "Noto Sans JP", sans-serif`,
                });
                textElements.push({
                ...
                textElements.push({
                    text: `${Number(item.scrobbles).toLocaleString("pt-BR")} ${
                        item.scrobbles == 1 ? "scrobble" : "scrobbles"
                    }`,
                    yOffset: config.SCROBBLE_TEXT_Y_OFFSET,
                    font:
                        `${config.TRACK_SCROBBLE_FONT_SIZE}px "Noto Sans JP", sans-serif`,
                });
            }
            ...
            if (param === "noplays") {
                textElements.push({
                    ...
                    yOffset: config.SCROBBLE_TEXT_Y_OFFSET +
                        config.TEXT_LINE_HEIGHT,
                    font:
                        `normal 700 ${config.ALBUM_NAME_FONT_SIZE}px "Noto Sans JP", sans-serif`,
                });
                textElements.push({
                    text: limitText(item.album.artist, 50),
                    yOffset: config.SCROBBLE_TEXT_Y_OFFSET,
                    font:
                        `normal 500 ${config.ALBUM_ARTIST_FONT_SIZE}px "Noto Sans JP", sans-serif`,
                });
            } else {
                textElements.push({
                    text: limitText(item.album.name, 50),
                ...
                textElements.push({
                    ...
                    yOffset: config.SCROBBLE_TEXT_Y_OFFSET,
                    font:
                        `${config.ALBUM_SCROBBLE_FONT_SIZE}px "Noto Sans JP", sans-serif`,
                });
            }
            ...
            if (param === "noplays") {
                textElements.push({
                    ...
                    yOffset: config.SCROBBLE_TEXT_Y_OFFSET,
                    font:
                        `normal 700 ${config.ARTIST_NAME_FONT_SIZE}px "Noto Sans JP", sans-serif`,
                });
            } else {
                textElements.push({
                    text: limitText(item.artist.name, 50),
                ...
                textElements.push({
                    ...
                    yOffset: config.SCROBBLE_TEXT_Y_OFFSET,
                    font:
                        `${config.ARTIST_SCROBBLE_FONT_SIZE}px "Noto Sans JP", sans-serif`,
                });
            }
        }
...
function gridlfTemplate(
    ...
    if (firstLarge) {
        ...
        const firstElements = generateScrobbleImage(
            firstItem,
            0,
            0,
            2 * config.POSTER_WIDTH,
            2 * config.POSTER_HEIGHT,
            param,
            config,
            media_type,
            true,
    ...
    while (index < lastfm_data.length) {
        ...
        const scrobbleElements = generateScrobbleImage(
            item,
            x,
            y,
            config.POSTER_WIDTH,
