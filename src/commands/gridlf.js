...
import {
    ...
    mediaMap,
    periodMap,
} from "../helpers/validValuesMap.js";
...
async function getLastfmData(lastfm_user, media_type, period, limit) {
    try {
        if (media_type === "tracks") {
            const lastfm_data = await getUserTopTracks(
                lastfm_user,
                period,
                limit,
            );
            ...
            for (const scrobble of lastfm_data) {
                ...
                let cached_image = await findCachedImage(mediaMap[media_type], {
                    ...lastfm_info,
                });

                if (!cached_image) {
                    const track_image_url = await getTrackImageUrl({
                        ...lastfm_info,
                        username: lastfm_user,
                    });
                    cached_image = await saveImageToCache(
                        mediaMap[media_type],
                        track_image_url,
                        lastfm_info,
                    );
        ...
        if (media_type === "albums") {
            const lastfm_data = await getUserTopAlbums(
                lastfm_user,
                period,
                limit,
            );
        ...
        if (media_type === "artists") {
            const lastfm_data = await getUserTopArtists(
                lastfm_user,
                period,
                limit,
            );
            ...
            for (const scrobble of lastfm_data) {
                ...
                if (!cached_image) {
                    const artist_image_url = await getArtistImageUrl({
                        ...lastfm_info,
                        username: lastfm_user,
                    });
                    cached_image = await saveImageToCache(
...
function parseArgs(args) {
    ...
    const media_type = args.find((arg) => acceptedMedias.includes(arg)) ||
        DEFAULT_MEDIA_TYPE;
    const period = args.find((arg) => acceptedPeriods.includes(arg)) ||
        DEFAULT_PERIOD;
    const param = args.find((arg) => ["notexts", "noplays"].includes(arg));
    const main = !args.includes("nomain");

    return { grid, media_type, period, param, main };
}
...
function validateGrid(grid) {
    ...
    if (
        COLUMNS > MAX_COLUMNS || COLUMNS < MIN_COLUMNS || ROWS > MAX_ROWS ||
        ROWS < MIN_ROWS
    ) {
...
async function gridlf(ctx) {
    ...
    try {
        ctx.replyWithChatAction("typing").catch((error) =>
            console.error(error)
        );
        ...
        const argInfos = parseArgs(args);
        const { grid, media_type, period, param } = argInfos;
        let main = argInfos.main;

        const { COLUMNS, ROWS } = validateGrid(grid);

        if (COLUMNS <= 2 && ROWS <= 2) main = false;
        ...
        const examplesCommand = [
            "/gridlf 4x1 tracks 3m",
            "/gridlf 5x3 art 6m",
            "/gridlf 10x10 notexts all",
            "/gridlf 5x5 noplays nomain 1y",
        ];
        const tipText =
            "💡 Tip: you can define your grid, type of media or make a collage with no text\n";
        ...
        responseExtra.entities.push(
            createEntity(
                responseMessage.indexOf(tipText),
                tipText.length,
                "italic",
            ),
        );
        for (const example of examplesCommand) {
            responseExtra.entities.push(
                createEntity(
                    responseMessage.indexOf(example),
                    example.length,
                    "code",
                ),
            );
        ...
        const lastfm_data = await getLastfmData(
            lastfm_user,
            mediaMap[media_type],
            periodMap[period],
            COLUMNS * ROWS,
        ...
        const extra = {
            reply_to_message_id: ctx.message?.message_id,
            caption: `${first_name}, your ${grid} ${
                mediaMap[media_type]
            } grid of ${periodMap[period]}`,
        };

        ctx.replyWithChatAction("upload_photo").catch((error) =>
            console.error(error)
        );
