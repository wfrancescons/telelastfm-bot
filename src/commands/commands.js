...
async function start(ctx) {
  ...
  try {
    ...
    const message = `Hello, ${first_name} 👋\n` +
      ...
      `\nOn Letterboxd? 🟠🟢🔵 Give @letterboxdgrambot a try\n` +
...
async function help(ctx) {
  ...
  try {
    ...
    for (const example in command_examples) {
      extra.entities.push(
        createEntity(
          message.indexOf(command_examples[example]),
          command_examples[example].length,
          "code",
        ),
      );
    }
...
async function privacy(ctx) {
  ...
  try {
    ...
    const message =
      `<strong>Termos de Uso e Política de Privacidade do telelastfm</strong>
